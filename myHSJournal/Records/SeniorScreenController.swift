...
class SeniorScreenController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    
    @IBOutlet weak var seniorTableView: UITableView!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.async {
            self.seniorTableView.reloadData() }
    }
    
    func setupTableView() {
        seniorTableView.delegate = self
        seniorTableView.dataSource = self
        
        self.seniorTableView.register(UINib.init(nibName: "HSRecTitleTableViewCell", bundle: .main), forCellReuseIdentifier: "HSRecTitleTableViewCell")
        self.seniorTableView.register(UINib.init(nibName: "HSRecDetailTableViewCell", bundle: .main), forCellReuseIdentifier: "HSRecDetailTableViewCell")
        self.seniorTableView.register(UINib.init(nibName: "HSRecNoValueTableViewCell", bundle: .main), forCellReuseIdentifier: "HSRecNoValueTableViewCell")
        
        seniorTableView.separatorStyle = UITableViewCell.SeparatorStyle.none
    }
        ...
        do {
            ...
            for (_, element) in hsItemArray.enumerated() {
                if (element.recType == "Academic" &&
                    element.schoolyear == SchoolYearType.TWELVETH.description) {
                    academicItemArray.append(element)
                } else if (element.recType == "Extracurricular" &&
                    element.schoolyear == SchoolYearType.TWELVETH.description) {
                    activityItemArray.append(element)
                } else if (element.schoolyear == SchoolYearType.TWELVETH.description) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if (indexPath.section == 0 && indexPath.row == 0) {
            let cell: HSRecTitleTableViewCell = seniorTableView.dequeueReusableCell(withIdentifier: "HSRecTitleTableViewCell", for: indexPath) as! HSRecTitleTableViewCell
            ...
            if (indexPath.section == 0) {
                if academicItemArray.count > 0 {
                    let cell: HSRecDetailTableViewCell = seniorTableView.dequeueReusableCell(withIdentifier: "HSRecDetailTableViewCell", for: indexPath) as! HSRecDetailTableViewCell
                    cell.setInputView(inputScreen: "SeniorScreen")
                    cell.configureCell(recItem: academicItemArray[indexPath.row-1], count: academicItemArray.count)
                    return cell
                } else {
                    let cell: HSRecNoValueTableViewCell = seniorTableView.dequeueReusableCell(withIdentifier: "HSRecNoValueTableViewCell", for: indexPath) as! HSRecNoValueTableViewCell
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let editAction = UITableViewRowAction(style: .default, title: "Edit", handler: { (action, indexPath) in
            self.performSegue(withIdentifier:"gotoSeniorRec", sender: self.seniorTableView.cellForRow(at: indexPath))
        })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.destination is SchoolRecordDetailsController {
            ...
            vc?.selectedSchoolYear = SchoolYearType.TWELVETH
            if sender != nil {
...
extension SeniorScreenController: GrowingCellProtocol {
    // Update height of UITextView based on string height
    func updateHeightOfRow(_ cell: HSRecDetailTableViewCell, _ textView: UITextView) {
        let size = textView.bounds.size
        let newSize = seniorTableView.sizeThatFits(CGSize(width: size.width,
                                                          height: CGFloat.greatestFiniteMagnitude))
        if size.height != newSize.height {
            UIView.setAnimationsEnabled(false)
            seniorTableView?.beginUpdates()
            seniorTableView?.endUpdates()
            UIView.setAnimationsEnabled(true)
            // Scoll up your textview if required
            if let thisIndexPath = seniorTableView.indexPath(for: cell) {
                seniorTableView.scrollToRow(at: thisIndexPath, at: .bottom, animated: false)
            }
        }
    }
}
